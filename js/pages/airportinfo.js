...
const AirportInfoPage = (() => {
  ...
  async function render() {
    ...
    const myCards = myAirports.map(ap => {
      ...
      return `<div class="ap-card${isActive?' is-active':''}" style="padding:0;overflow:hidden">
        ${Images.airportCardImg(ap.iata, '110px')}
        <div style="padding:14px 16px">
        ${isActive?'<div class="ap-active-badge" style="top:120px">Active</div>':''}
        <div style="cursor:pointer;margin-bottom:8px" data-detail="${ap.iata}">
          <div style="font-size:22px;font-weight:800;color:var(--navy)">${ap.iata}</div>
    ...
    const allCards = allIatas.map(iata => {
      ...
      return `<div class="ap-card" style="${inMyList?'opacity:0.55':''}">
        <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:8px">
          <div>
            <div style="font-size:20px;font-weight:800;color:var(--navy)">${iata}</div>
  ...
  async function renderDetail(iata, layout, chars) {
    ...
    <div style="display:flex;align-items:center;gap:12px;margin-bottom:16px">
      <button class="btn btn-outline btn-sm" id="btn-back-list">← Back</button>
      <div>
        <h2 style="font-size:18px;font-weight:700">${L.name||iata} (${iata})</h2>
        <p style="font-size:13px;color:var(--text2)">Full airport profile from game data</p>
      </div>
