...
const DestinationsPage = (() => {
  ...
  async function render() {
    ...
    // card() is module-level — see below
    ...
    <div class="toolbar">
      ...
      <span id="dest-count" style="font-size:12px;color:var(--text2);margin-left:auto;white-space:nowrap">${items.length} shown</span>
    </div>
    ${content}`;
  }

  function reGrid() {
    const data = _data;  // already loaded, use cached
    if (!data) return;
    const items = filtered(data);
  ...
  function bindEvents() {
    ...
    document.getElementById('dq')?.addEventListener('input', e => {
      ...
      _debounce = setTimeout(reGrid, 150);  // 150ms debounce — feels instant
    });
    document.getElementById('dsz')?.addEventListener('change', e => {
...
const ReferencePage = (() => {
  ...
  async function render() {
    ...
    <div class="toolbar">
      ...
      <span id="ref-count" style="font-size:12px;color:var(--text2);margin-left:auto;white-space:nowrap">${items.length} shown</span>
    </div>
    <div class="table-wrap"><table>
      <thead><tr>
