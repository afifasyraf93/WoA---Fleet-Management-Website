const WishlistPage = (() => {
  ...
  function card(w) {
    return `<div class="card" data-id="${w.id}" style="padding:0;overflow:hidden;display:flex;flex-direction:column">
      ${Images.aircraftCardImg(w.icao, w.size, w.type, '160px')}
  ...
  function detail(w) {
    const bg = {'A':'#dbeafe','B':'#e0f2fe','C':'#ede9fe','D':'#fef3c7','E':'#fce7f3','F':'#fee2e2'}[w.size]||'#f0f4f8';
    return `<div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;align-items:start">
      <div style="background:${bg};border-radius:var(--radius-lg);overflow:hidden;
        ...
        <img src="images/aircraft/${w.icao}.png" alt="${w.icao}"
          style="width:100%;max-height:260px;object-fit:contain;display:block"
          onerror="this.style.display='none';this.nextElementSibling.style.display='flex'">
        <div style="display:none;flex-direction:column;align-items:center;justify-content:center;padding:40px;font-size:48px">
          ${w.type==='Cargo'?'📦':'✈️'}
        </div>
        <div style="padding:12px 16px;text-align:center;border-top:1px solid rgba(0,0,0,.06);width:100%;background:rgba(255,255,255,.5)">
          <div style="font-size:18px;font-weight:800;color:var(--text)">${w.icao}</div>
          <div style="font-size:12px;color:var(--text2)">${w.name}</div>
          <div style="display:flex;gap:5px;justify-content:center;margin-top:6px">
            ${bSize(w.size)}${bType(w.type)}
          </div>
        </div>
      </div>
      <div style="background:var(--surface);border:1px solid var(--border);border-radius:var(--radius-lg);padding:14px">
        <div style="font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:var(--text3);margin-bottom:10px">Wishlist Details</div>
        <div class="detail-grid" style="grid-template-columns:1fr">
          ...
          <div class="di"><div class="dl">Planned Route</div><div class="dd">${w.plannedRoute||'—'}</div></div>
          <div class="di"><div class="dl">Stand Type</div><div class="dd">${w.standType||'—'}</div></div>
          ${w.notes?`<div class="di"><div class="dl">Notes</div><div class="detail-note">${w.notes}</div></div>`:''}
        </div>
      </div>
    </div>`;
  }
  ...
  function openModal(mode, id=null) {
    ...
    if(mode==='view'){
      ...
      document.getElementById('wish-modal')?.querySelector('.modal')?.style && (document.getElementById('wish-modal').querySelector('.modal').style.maxWidth='820px');
      ...
      const wmi=document.getElementById('wish-modal')?.querySelector('.modal');
      if(wmi) wmi.style.maxWidth='540px';
      body.innerHTML=form({});
